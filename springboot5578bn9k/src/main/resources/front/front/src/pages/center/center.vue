	...
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="info-item5" v-if="userTableName=='yonghu'">
				...
				<div class="text">{{sessionForm.shengao}}</div>
			</div>
			<div class="info-item6" v-if="userTableName=='yonghu'">
				<span class="icon iconfont icon-shouye-zhihui"></span>
				<div class="label">体重：</div>
				<div class="text">{{sessionForm.tizhong}}</div>
			</div>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="120px">
					...
					<el-form-item class="center-item" v-if="userTableName=='yonghu'" label="身高" prop="shengao">
						<el-input v-model="sessionForm.shengao" placeholder="身高" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yonghu'" label="体重" prop="tizhong">
						<el-input v-model="sessionForm.tizhong" placeholder="体重" ></el-input>
					</el-form-item>
					<el-form-item class="center-item" v-if="userTableName=='yonghu'" label="运动习惯" prop="yundongxiguan">
						<el-input v-model="sessionForm.yundongxiguan" placeholder="运动习惯" ></el-input>
					</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('yonghu' == this.userTableName) {
				this.$set(this.sessionForm, 'yundongxiguan', null);
			}
			if ('yonghu' == this.userTableName) {
			...
			if ('yonghu' == this.userTableName&&this.rules['shengao']){
				this.rules['shengao'].push({ required: true, message: '请输入身高', trigger: 'blur' })
			}else if('yonghu' == this.userTableName&&!this.rules['shengao']) {
				this.$set(this.rules, 'shengao', [{ required: true, message: '请输入身高', trigger: 'blur' }]);
			}
			if ('yonghu' == this.userTableName&&this.rules['tizhong']){
				this.rules['tizhong'].push({ required: true, message: '请输入体重', trigger: 'blur' })
			}else if('yonghu' == this.userTableName&&!this.rules['tizhong']) {
				this.$set(this.rules, 'tizhong', [{ required: true, message: '请输入体重', trigger: 'blur' }]);
			}
			if ('yonghu' == this.userTableName&&this.rules['yundongxiguan']){
				this.rules['yundongxiguan'].push({ required: true, message: '请输入运动习惯', trigger: 'blur' })
			}else if('yonghu' == this.userTableName&&!this.rules['yundongxiguan']) {
				this.$set(this.rules, 'yundongxiguan', [{ required: true, message: '请输入运动习惯', trigger: 'blur' }]);
			}
			if ('yonghu' == this.userTableName) {
