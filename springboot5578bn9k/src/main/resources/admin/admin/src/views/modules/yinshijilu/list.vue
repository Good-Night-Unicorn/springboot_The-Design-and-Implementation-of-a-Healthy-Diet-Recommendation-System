			...
			<el-form class="center-form-pv" :style='{"margin":"0 0px 20px 0","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#000000","textAlign":"right","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"400","height":"40px"}' class="item-label">日期</label>
					...
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}' class="select" label="餐次" prop="canci">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#000000","textAlign":"right","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"400","height":"40px"}' class="item-label">餐次</label>
					...
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}' class="select">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#000000","textAlign":"right","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"400","height":"40px"}' class="item-label">是否通过</label>
				...
				<el-row class="actions" :style='{"padding":"10px 0","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yinshijilu','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						审核
					</el-button>
				</el-row>
			</el-form>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="edit" v-if=" isAuth('yinshijilu','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
	...
	export default {
		...
		methods: {
			...
			init () {
				...
				this.canciOptions = "早餐,中餐,晚餐,加餐".split(',')
			},
			search() {
				this.pageIndex = 1;
				this.getDataList();
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "yinshijilu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
										...
										for(let x in this.shList){
											await this.$http.get('yonghu/query',{params: {
												...
												let param = {
													...
													content: `你提交的“饮食记录”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
