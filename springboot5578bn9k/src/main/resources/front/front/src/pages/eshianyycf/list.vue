		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">中文名：</div>
					<el-input v-model="formSearch.cname" placeholder="中文名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">食品分类：</div>
					<el-input v-model="formSearch.spfenlei" placeholder="食品分类" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('eshianyycf','新增')" type="primary" @click="add('/index/eshianyycfAdd')">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<div class="name">中文名:{{item.cname}}</div>
						<div class="name">英文名:{{item.ename}}</div>
						<div class="name">食品分类:{{item.spfenlei}}</div>
						<div class="time_item">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.cname != '') searchWhere.cname = '%' + this.formSearch.cname + '%';
				if (this.formSearch.spfenlei != '') searchWhere.spfenlei = '%' + this.formSearch.spfenlei + '%';
				if (this.formSearch.chandi != '') searchWhere.chandi = '%' + this.formSearch.chandi + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`eshianyycf/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/eshianyycfDetail', query: params});
			},
